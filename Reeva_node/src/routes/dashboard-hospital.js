...
async function fetchBoxAggregates(selectedEmpresaId = null) {
  ...
  const espaciosTable = tables.espacios;
  ...
  // Traer mesas desde tabla de espacios para asegurar conteo correcto por empresa
  let mesasFromEspacios = [];
  if (selectedEmpresaId && espaciosTable) {
    try {
      const espaciosResult = await db.send(
        new QueryCommand({
          TableName: espaciosTable,
      ...
      mesasFromEspacios = espacios.flatMap((espacio) => {
        if (!Array.isArray(espacio.mesas)) return [];
        return espacio.mesas.map((mesa) => ({
          idBox: mesa.id || mesa.mesaId || mesa.numero || "",
          numero: mesa.numero ?? mesa.nombre ?? mesa.id ?? "",
          idTipoBox: null,
          idEstadoBox: null,
          pasillo: espacio.pasilloNombre || espacio.nombre || "",
          piso: espacio.piso || "",
        }));
      });
    } catch (err) {
      console.warn("No se pudieron cargar mesas desde espacios:", err.message);
    }
  }

  // Combinar boxes reales con mesas faltantes (para conteo)
  const knownBoxIds = new Set(boxesRaw.map((b) => String(b.idBox ?? b.id ?? "")));
  const mesasNuevas = mesasFromEspacios.filter((mesa) => {
    const id = String(mesa.idBox || "");
    return id && !knownBoxIds.has(id);
  });

  const boxesCombinedRaw = boxesRaw.concat(mesasNuevas);

  const boxes = boxesCombinedRaw.map((box) => {
